...
const PROBE_FAST_THRESHOLD_MS = 10000;
const PROBE_SLOW_THRESHOLD_MS = 30000;
...
function probeSucceeded(record) {
  ...
  return Boolean(record.is_probe_success ?? record.isProbeSuccess)
    && category !== "timeout"
    && !(record.is_timeout ?? record.isTimeout);
}

function probeDurationMs(record) {
  const rawValue = record.duration_ms ?? record.durationMs;
  const fallback = record.source_kind === "probe" ? record.first_token_ms : undefined;
  const value = Number(rawValue ?? fallback);
  return Number.isFinite(value) ? value : null;
}

function probeTimelineClass(record) {
  if (!probeSucceeded(record)) return "is-failure";
  const duration = probeDurationMs(record);
  if (duration === null || duration < PROBE_FAST_THRESHOLD_MS) return "is-success";
  if (duration < PROBE_SLOW_THRESHOLD_MS) return "is-warning";
  return "is-failure";
}
...
function probeTimelineDetail(record) {
  ...
  const duration = probeDurationMs(record);
  const durationText = duration === null ? "耗时 -" : `耗时 ${formatDuration(duration)}`;
  const scoreText = `评分 ${evidenceScoreText(record)}`;
  if (probeSucceeded(record)) return `探活成功 · ${durationText} · ${scoreText} · ${time}`;
  const category = evidenceCategoryText(record.category);
  const detail = evidenceDetail(record);
  return [`探活失败（${category}）`, detail === "-" ? "无错误详情" : detail, scoreText, time].filter(Boolean).join(" · ");
}
  ...
  <section v-loading="loading" class="dispatch-page">
    ...
    <section v-loading="policyLoading" class="policy-panel">
      ...
      <div class="policy-rules">
        ...
        <p><strong>证据分值</strong><span>请求成功为 100 分；主动探活低于 10 秒为 100 分，10 至 30 秒为 60 分，30 秒及以上视为超时并计 10 分，探活失败为 10 分；成功但首字耗时超过 {{ Number(policyConfig.slow_first_token_ms) / 1000 }} 秒为 65 分；认证失败（401/403）、余额不足或用量上限异常为 0 分；超时为 10 分；HTTP 429/502/503 为 25 分；其他上游错误为 40 分。每条分值按“系统计算”的短期和长期占比合成健康分，上游价格不参与健康分计算。</span></p>
                ...
                <div class="probe-timeline-bars">
                  ...
                  >
                    <button
                      ...
                      :class="probeTimelineClass(record)"
