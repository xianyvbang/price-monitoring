  ...
  <section v-loading="loading" class="dispatch-page">
    ...
    <section v-loading="policyLoading" class="policy-panel">
      ...
      <div class="policy-rules">
        ...
        <p><strong>证据分值</strong><span>请求成功为 100 分；主动探活低于 10 秒为 100 分，10 至 30 秒为 50 分，探活失败为 10 分；成功但首字耗时超过 {{ Number(policyConfig.slow_first_token_ms) / 1000 }} 秒为 65 分；认证失败（401/403）、余额不足或用量上限异常为 0 分；超时为 10 分；HTTP 429/502/503 为 25 分；其他上游错误为 40 分。每条分值按“系统计算”的短期和长期占比合成健康分，上游价格不参与健康分计算。</span></p>
