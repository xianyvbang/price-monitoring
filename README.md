...
“平台调度”页面使用 SQLite 中最后一次成功同步的账号集合。手动同步只查询 Sub2API 的 API Key 账号，并按账号 ID 增量合并：已有 API Key 账号保留历史字段，只补充新发现的分组归属；新账号加入集合，本次未返回的历史账号不会删除；历史缓存中已明确标记为其他类型的账号会在下次成功同步时移除。每个已知分组都可单独同步，全部同步仍可按平台、状态及是否包含未分组账号筛选本次拉取范围。远端请求失败时保留旧集合；账号手动启停会立即写入 Sub2API 并更新本地快照。

自动评分默认开启；关闭后，后台不再自动读取证据、执行探活或重算健康分，但手动证据刷新和立即执行仍可使用。自动调度总开关和健康回池、智能扩容、负载因子、价格保护四项策略默认关闭；开启自动调度会同时开启自动评分，关闭自动评分会同时关闭自动调度。仅开启自动评分时，后台会增量读取请求证据、按最近一次探活时间判断到期探活并计算健康评分，但不写远端；开启自动调度后，异常账号停用和至少保留 1 个可用账号始终生效，即使四项子策略均关闭。调度器默认每 60 秒运行一轮，可在页面的规则参数中调整（最短 5 秒）；健康异常策略每轮每个分组最多自动关闭 1 个账号的调度，恢复时会在同一轮自动开启全部符合条件的账号。多分组账号关闭后会占用其所属全部分组的当轮关闭名额。账号健康证据每个账号保留最近 60 条，探活证据有效期默认 180 秒，账号状态、目标并发、目标负载因子和操作审计均保存在 SQLite。
...
价格保护使用账号所属本地平台分组中的最小正数倍率作为销售倍率，最低安全销售倍率为 `上游成本倍率 × (1 + 最低利润率 / 100)`，最低利润率默认 10%。低于安全线的全部账号会在同一轮立即关闭；成本超过宽限期时只标记倍率过期并停止成本调权，不会关闭调度。未绑定或本地销售倍率未知时只告警，不推断为 1.0。价格重新安全且健康证据有效时，系统会在同一轮恢复全部符合条件的 active 账号，包括价格保护、健康策略或人工关闭的账号；远端 inactive/error 账号不会自动恢复。关闭价格保护开关不会主动恢复此前关闭的账号。
